...
module.exports = async(function swagger_editor_cli(nopter, args, basedir, cancellationToken) {	
	...
	server.on('request', function (req, res) {
		res.on('finish', function () {
			var logmsg = res.statusCode + ' ' + req.method + ' ' + req.originalUrl;
			if (res.statusCode >= 500) {
				console.log(colors.red(logmsg));
			} else if (res.statusCode >= 400) {
				console.log(colors.yellow(logmsg));
			} else if (req.method === 'GET') {
				console.log(colors.grey(logmsg));
			} else {
				console.log(logmsg);
			}
		});
	...
	console.log(colors.green('Server is running.. ' + url));
	open(url);
	...
	console.log('Shutting down http server..'.yellow);
